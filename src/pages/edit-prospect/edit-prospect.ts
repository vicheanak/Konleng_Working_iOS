 ...
 */
 ...
 export class EditProspectPage {
 	...
 	ionViewWillEnter(){ 
 		...
 		if (this.navParams.get('listing')){
 			...
 			this.auth.getUser().then((user) => {
 				this.user = user;
 				this.listingProvider.getUserListings(this.user.uid, 3, 1).then((listing) => {
 					console.log('hi');
 	...
 	delete(){
 		this.presentLoading();
 		this.listingProvider.deleteProspect(this.prospect.id).then((transaction) => {
 			this.dismissLoading();
 		});
 	}
 	...
 	presentListingModal() {
 		let listingModal = this.modalCtrl.create(EditProspectListingModal);
 ...
 export class EditProspectListingModal {
